...
-- 집합 연산자 (SET OPERATOR)
-- 합집함 : UNION, UNION ALL
--  여러 개의 SELECT 결과를 하나로 합침
-- 교집합 : INTERSECT
--  여러 개의 SELECT 결과에서 공통된 부분만 결과로 추출함
-- 차집합 : MINUS
--  첫번째 SELECT 한 결과에서 두번째 SELECT 한 결과와 겹치는 부분을 뺀 결과
...
-- 집합 연산자 사용시 주의사항
-- 여러 개의 SELECT 절에 나열된 컬럼갯수가 반드시 같아야 함 : DUMMY COLUMN 사용
-- 각 SELECT 절에 나열된 컬럼별 자료형도 같아야 함

SELECT EMP_NAME, JOB_ID, HIRE_DATE
FROM EMPLOYEE
WHERE DEPT_ID = '20'
UNION
--SELECT DEPT_NAME, DEPT_ID -- 갯수가 다름, ERROR
SELECT DEPT_NAME, DEPT_ID, NULL -- DUMMY COLUMN 사용. 갯수 맞춤
--SELECT DEPT_NAME, NULL, DEPT_ID -- 자료형이 다름, ERROR
FROM DEPARTMENT
WHERE DEPT_ID = '20';

-- 50번 부서에 소속된 직원중 관리자와 일반 직원을 따로 조회해서 하나로 합치시오.
SELECT *
FROM EMPLOYEE
WHERE DEPT_ID = '50';  -- 50번 부서원들 조회해 봄
...
FROM EMPLOYEE
WHERE DEPT_ID = '50' AND EMP_ID <> '141'
ORDER BY 3, 1;  -- 맨 마지막에 딱 한번 사용할 수 있음

-- 리터럴(LITERAL)에 대한 집합연산자 처리도 가능함
SELECT 'SQL을 공부하고 있습니다' 문장, 3 순서 FROM DUAL
UNION
SELECT '우리는 지금', 1 FROM DUAL
UNION
SELECT '아주 재미있게', 2 FROM DUAL
ORDER BY 2;  -- 리터럴 별칭 사용할 수 없음. 순번만 사용 가능함
